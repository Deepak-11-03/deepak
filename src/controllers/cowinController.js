...
let getCitiesSorted = async function (req, res) {
    try {
        ...
        for (let i = 0; i < cities.length; i++) {
            ...
            let options = {
                method: "get",
                url: `http://api.openweathermap.org/data/2.5/weather?q=${cities[i]}&appid=01d01458b8dff37873dd9ec805af74d8`
            }
            let result = await axios(options);
...
let createMemes = async function (req, res) {
    try {
        ...
        let text0 = data.text0
        let text1 = data.text1
        let user = data.username
        let pass = data.password
        var options = {
            method: "post",
            url: `https://api.imgflip.com/caption_image?template_id=${template}&text0=${text0}&text1=${text1}&username=${user}&password=${pass}`
        }
